...
void ogs_sbi_context_init(OpenAPI_nf_type_e nf_type)
{
    ...
    if (ogs_app()->pool.nf <= 0)
        ogs_app()->pool.nf = 1;
    if (ogs_app()->pool.nf_service <= 0)
        ogs_app()->pool.nf_service = ogs_app()->pool.nf;
    if (ogs_app()->pool.subscription <= 0)
        ogs_app()->pool.subscription = ogs_app()->pool.nf;
    ...
    ogs_pool_init(&nf_info_pool,
            ogs_app()->pool.nf * (OGS_MAX_NUM_OF_NF_INFO ? OGS_MAX_NUM_OF_NF_INFO : 1));
